...
package Interfaces.LPC4337.SCU is
   ...
   type SFS_Register(Pin : Pin_Type := Normal_Drive) is record
      ...
      --  Select pin function.
      MODE     : MODE_ENUM := Interfaces.LPC4337.SCU.Function_0_Default;

      --  Enable pull-down resistor at pad.
      EPD      : EPD_ENUM := Interfaces.LPC4337.SCU.Disable_Pull_Down;

      --  Disable pull-up resistor at pad. By default, the pull-up resistor is
      --  enabled at reset.
      EPUN     : EPUN_ENUM := Interfaces.LPC4337.SCU.Enable_Pull_Up;

      --  Input buffer enable. The input buffer is disabled by default
      --  at reset and must be enabled for receiving.
      EZI      : EZI_ENUM := Interfaces.LPC4337.SCU.Disable_Input_Buffer;

      --  Input glitch filter. Disable the input glitch filter for
      --  clocking signals higher than 30 MHz.
      ZIF      : ZIF_ENUM := Interfaces.LPC4337.SCU.Enable_Input_Glitch;

      --  Reserved
      RESERVED     : Interfaces.LPC4337.UInt22 := 16#0#;

      case Pin is
         when Normal_Drive =>

            --  Reserved
            ND_EHS_RESERVED : Interfaces.LPC4337.Bit := 16#0#;
            --  Reserved
            ND_EHD_RESERVED : Interfaces.LPC4337.UInt2 := 16#0#;

         when High_Drive =>

            --  Select drive strength.
            EHD : EHD_ENUM := Interfaces.LPC4337.SCU.Normal_Drive_4_Ma_D;
            --  Reserved
            HD_EHS_RESERVED : Interfaces.LPC4337.Bit := 16#0#;

         when High_Speed =>

            --  Select Slew rate.
            EHS    : EHS_ENUM := Interfaces.LPC4337.SCU.Slow_Low_Noise_With;
            --  Reserved
            HS_EHD_RESERVED : Interfaces.LPC4337.UInt2 := 16#0#;
